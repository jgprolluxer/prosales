angular.module('prosales-app')
    .controller('posController', function ($scope, $http, DTOptionsBuilder, DTColumnDefBuilder) {
            ...
            $http.post("/OrderProducts/jsCancelOrderProduct", orderProduct).success(function (data)
            {
                if (data)
                {
                    if (data["success"])
                    {
                        ...
                        alert('El producto no se cancelo por las siguientes razones <br/>' + data["message"]);
                    }
                }
            }).error(function(data, status, headers, config)
            {
        ...
        if( stringToBoolean( $('#orderID').val() ) )
        {
            $scope.newOrder = {
                Order:{
                    id: $('#orderID').val()
