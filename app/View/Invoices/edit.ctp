...
<div class="container-fluid">
    ...
    <div class="row-fluid">
       <div class="span12">
		 <div class="btn-group">
			<p><a href="<?php echo Router::url("/invoices/delete/" . $this->data["Invoice"]["id"]); ?>" class="btn red"><i class="icon-trash"></i> <?php echo __('Cancelar Factura'); ?></a>
		 </p>     
          </div>		   
             ...
             <div class="portlet-body form">
				...
				<div class="row-fluid">
                   <div class="span6 ">
			            ...
			            <?php else: ?>  	         
		                	<?php echo $this->Form->input('order_name', array("readonly"=>"true","id"=>"quote_name", "value"=>$orders["name"], 
		                		'label' => array('class' => 'control-label','text'=>__('Pedido')),
		                		'after' => '<span></span></div>'
		              		  )); ?> 		            
			             <?php endif; ?>  	         
                   ...
                   <div class="span6 ">    
		   		        <?php echo $this->Form->input('status', array('type'=>'select', 'options'=> $invoiceStatus,
		                	'label' => array('class' => 'control-label','text'=>__('Estatus'))
		                )); ?>			                        
	 		            <?php echo $this->Form->input('invoice_date', array('type'=>'text','class'=>'m-wrap m-ctrl-medium date-picker',
	                		'label' => array('class' => 'control-label','text'=>__('Fecha de Factura'))
	                	)); ?>
	 		            <?php echo $this->Form->input('due_date', array('type'=>'text','class'=>'m-wrap m-ctrl-medium date-picker',
				...
				<div class="row-fluid">
  				...
  				<div class="row-fluid">
                 	<div class="span6 "> 	  
   	                	<?php echo $this->Form->input('tax', array(
	...
	<div class="tabbable tabbable-custom">
		<ul class="nav nav-tabs">
			<li class="active"><a href="#tab_1_1" data-toggle="tab">Actividades</a></li>
		</ul>
		<div class="tab-content">
			<div class="tab-pane active" id="tab_1_1">				
				<?php 
					echo $this->element('DataTables/related_activities');
				?>
			</div>
		</div>
	</div> 
 ...
 jQuery(document).ready(function () {
 	...
 	function updateQuoteInput() {
	 	if($('#account_id').val() =="") {
	 		...
	 		$('#quote_name').attr("placeholder","Por favor selecione una cuenta...");
	 		$('#quote_name').attr("disabled",true);
	 	}
	 	else {
	 		$('#quote_name').attr("placeholder","");
	 		$('#quote_name').attr("disabled",false);	 	
	 	}		
 	}
    ...
    jQuery( "#quote_name" ).autocomplete({
	  messages: {
      ...
      source: function( request, response ) {
        $.ajax({      
          url: qualifyURL("/Quotes/findQuote"),
          dataType: "json",
          data: {
