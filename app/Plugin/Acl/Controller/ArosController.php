...
class ArosController extends AclAppController {
    ...
    function admin_permissions_by_role($elRoleName = "", $page=0) {
    	
    	$elRolefilter = "";
    	
    	if(!empty($this->request->query)) {
    		$elRolefilter = $this->request->query["roleFilter"];
    	}
    	$this->set('currRoleFilter', $elRolefilter);
        ...
        if ("" !== $elRoleName) {
            $roles = $this->{$role_model_name}->find('all', array(
                'conditions' => array(
                    $role_model_name . '.' . $role_display_field . ' =' => $elRoleName
                ),
                'order' => $role_display_field, 'contain' => false, 'recursive' => -1
            ));
        }
        
        $actions = $this->AclReflector->get_all_actions();
        
		//2014.09.10 SFICHERA: Added filter by name...
        $actions = array_filter($actions,
	        function($val) use ($elRolefilter) {
	        	return preg_match("#" . strtoupper($elRolefilter) . "#", strtoupper($val));
	        }
		);
		
		$this->{$role_model_name}->recursive = -1;
		$allroles = $this->{$role_model_name}->find('list',
					array('order' => $role_display_field,
						'fields' => array($role_model_name.'.id', $role_model_name.'.name'),
						'contain' => false, 
						'conditions' => array($role_model_name .'.name !=' => $elRoleName),
						'recursive' => -1
					)
		);
        ...
        $pagi	= $page * $this->paginate['limit'];
        $i		= 0;
        $curri	= 0;
        
        $this->set('totalPages', round((count($actions) / $this->paginate['limit']),0,PHP_ROUND_HALF_UP));
            ...
            if ($controller_name != 'App') {
            	...
            	if($i > $pagi ) {
	                ...
	                if (isset($plugin_name)) {
	                	...
	                	/*if(!isset($methods['app'][$controller_name])) {
    ...
    function admin_copy_role_permissions() {
    	
    	$roleFrom 		= $this->request->data["roleCopyFrom"];
    	$roleTo 		= $this->request->data["roleCopyTo"];
    	$currentRole 	= $this->request->data["currentRole"];
    	
		$q = "DELETE FROM aros_acos WHERE aros_acos.aro_id = (SELECT id FROM aros WHERE aros.foreign_key = " . $roleTo . " AND aros.model ='GROUP');";
		$db = ConnectionManager::getDataSource('default');
		$myData = $db->query($q);

		$q = "INSERT INTO aros_acos (aro_id, aco_id,_create, _read, _update, _delete)";
		$q = $q . "SELECT (SELECT id FROM aros WHERE aros.foreign_key = " . $roleTo . " AND aros.model ='GROUP'),";
  		$q = $q . "aros_acos.aco_id, aros_acos._create,aros_acos._read, aros_acos._update, aros_acos._delete";
 		$q = $q . " FROM aros_acos WHERE aros_acos.aro_id = (SELECT id FROM aros WHERE aros.foreign_key = " . $roleFrom . " AND aros.model ='GROUP');";
		$db = ConnectionManager::getDataSource('default');
		$myData = $db->query($q);

    	
    	$this->redirect("/admin/acl/aros/permissions_by_role/" . $currentRole);
