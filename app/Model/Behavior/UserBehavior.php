...
class UserBehavior extends ModelBehavior
{
    ...
    private function setCreatedUpdated($Model)
    {
        ...
        if (isset($loggedUser))
        {
            if ($Model->id)
            {
                $Model->data['User']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['User']['updated_by'])))
                {
                    $Model->data['User']['updated_by'] = $loggedUser["id"];
                }
            } else
            {
                $Model->data['User']['created'] = date("Y-m-d H:i:s");
                $Model->data['User']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['User']['updated_by'])))
                {
                    $Model->data['User']['updated_by'] = $loggedUser["id"];
                }
                if (!(isset($Model->data['User']['created_by'])))
                {
                    $Model->data['User']['created_by'] = $loggedUser["id"];
                }
            }
        }
        return $Model;
