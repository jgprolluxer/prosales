...
class LovBehavior extends ModelBehavior
{
    ...
    private function setCreatedUpdated($Model)
    {
        ...
        if (isset($loggedUser))
        {
            if ($Model->id)
            {
                $Model->data['Lov']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Lov']['updated_by'])))
                {
                    $Model->data['Lov']['updated_by'] = $loggedUser["id"];
                }
            } else
            {
                $Model->data['Lov']['created'] = date("Y-m-d H:i:s");
                $Model->data['Lov']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Lov']['updated_by'])))
                {
                    $Model->data['Lov']['updated_by'] = $loggedUser["id"];
                }
                if (!(isset($Model->data['Lov']['created_by'])))
                {
                    $Model->data['Lov']['created_by'] = $loggedUser["id"];
                }
            }
        }
        return $Model;
