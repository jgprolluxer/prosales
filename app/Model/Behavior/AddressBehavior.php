...
class AddressBehavior extends ModelBehavior
{
    ...
    private function setCreatedUpdated($Model)
    {
        ...
        if (isset($loggedUser))
        {
            if ($Model->id)
            {
                $Model->data['Address']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Address']['updated_by'])))
                {
                    $Model->data['Address']['updated_by'] = $loggedUser["id"];
                }
            } else
            {
                $Model->data['Address']['created'] = date("Y-m-d H:i:s");
                $Model->data['Address']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Address']['updated_by'])))
                {
                    $Model->data['Address']['updated_by'] = $loggedUser["id"];
                }
                if (!(isset($Model->data['Address']['created_by'])))
                {
                    $Model->data['Address']['created_by'] = $loggedUser["id"];
                }
            }
        }
        return $Model;
