...
class ResourceBehavior extends ModelBehavior
{
    ...
    private function setCreatedUpdated($Model)
    {
        ...
        if (isset($loggedUser))
        {
            if ($Model->id)
            {
                $Model->data['Resource']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Resource']['updated_by'])))
                {
                    $Model->data['Resource']['updated_by'] = $loggedUser["id"];
                }
            } else
            {
                $Model->data['Resource']['created'] = date("Y-m-d H:i:s");
                $Model->data['Resource']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Resource']['updated_by'])))
                {
                    $Model->data['Resource']['updated_by'] = $loggedUser["id"];
                }
                if (!(isset($Model->data['Resource']['created_by'])))
                {
                    $Model->data['Resource']['created_by'] = $loggedUser["id"];
                }
            }
        }
        return $Model;
