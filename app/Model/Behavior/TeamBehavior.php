...
class TeamBehavior extends ModelBehavior
{
    ...
    private function setCreatedUpdated($Model)
    {
        ...
        if (isset($loggedUser))
        {
            if ($Model->id)
            {
                $Model->data['Team']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Team']['updated_by'])))
                {
                    $Model->data['Team']['updated_by'] = $loggedUser["id"];
                }
            } else
            {
                $Model->data['Team']['created'] = date("Y-m-d H:i:s");
                $Model->data['Team']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Team']['updated_by'])))
                {
                    $Model->data['Team']['updated_by'] = $loggedUser["id"];
                }
                if (!(isset($Model->data['Team']['created_by'])))
                {
                    $Model->data['Team']['created_by'] = $loggedUser["id"];
                }
            }
        }
        return $Model;
