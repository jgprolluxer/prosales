...
class FamilyBehavior extends ModelBehavior
{
    ...
    private function setCreatedUpdated($Model)
    {
        ...
        if (isset($loggedUser))
        {
            if ($Model->id)
            {
                $Model->data['Family']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Family']['updated_by'])))
                {
                    $Model->data['Family']['updated_by'] = $loggedUser["id"];
                }
            } else
            {
                $Model->data['Family']['created'] = date("Y-m-d H:i:s");
                $Model->data['Family']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Family']['updated_by'])))
                {
                    $Model->data['Family']['updated_by'] = $loggedUser["id"];
                }
                if (!(isset($Model->data['Family']['created_by'])))
                {
                    $Model->data['Family']['created_by'] = $loggedUser["id"];
                }
            }
        }
        return $Model;
