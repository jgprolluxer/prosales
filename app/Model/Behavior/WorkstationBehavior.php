...
class WorkstationBehavior extends ModelBehavior
{
    ...
    private function setCreatedUpdated($Model)
    {
        ...
        if (isset($loggedUser))
        {
            if ($Model->id)
            {
                $Model->data['Workstation']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Workstation']['updated_by'])))
                {
                    $Model->data['Workstation']['updated_by'] = $loggedUser["id"];
                }
            } else
            {
                $Model->data['Workstation']['created'] = date("Y-m-d H:i:s");
                $Model->data['Workstation']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Workstation']['updated_by'])))
                {
                    $Model->data['Workstation']['updated_by'] = $loggedUser["id"];
                }
                if (!(isset($Model->data['Workstation']['created_by'])))
                {
                    $Model->data['Workstation']['created_by'] = $loggedUser["id"];
                }
            }
        }
        return $Model;
