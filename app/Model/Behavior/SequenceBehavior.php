...
class SequenceBehavior extends ModelBehavior
{
    ...
    private function setCreatedUpdated($Model)
    {
        ...
        if (isset($loggedUser))
        {
            if ($Model->id)
            {
                $Model->data['Sequence']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Sequence']['updated_by'])))
                {
                    $Model->data['Sequence']['updated_by'] = $loggedUser["id"];
                }
            } else
            {
                $Model->data['Sequence']['created'] = date("Y-m-d H:i:s");
                $Model->data['Sequence']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Sequence']['updated_by'])))
                {
                    $Model->data['Sequence']['updated_by'] = $loggedUser["id"];
                }
                if (!(isset($Model->data['Sequence']['created_by'])))
                {
                    $Model->data['Sequence']['created_by'] = $loggedUser["id"];
                }
            }
        }
        return $Model;
