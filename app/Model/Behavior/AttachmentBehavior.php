...
class AttachmentBehavior extends ModelBehavior
{
    ...
    private function setCreatedUpdated($Model)
    {
        ...
        if (isset($loggedUser))
        {
            if ($Model->id)
            {
                $Model->data['Attachment']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Attachment']['updated_by'])))
                {
                    $Model->data['Attachment']['updated_by'] = $loggedUser["id"];
                }
            } else
            {
                $Model->data['Attachment']['created'] = date("Y-m-d H:i:s");
                $Model->data['Attachment']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Attachment']['updated_by'])))
                {
                    $Model->data['Attachment']['updated_by'] = $loggedUser["id"];
                }
                if (!(isset($Model->data['Attachment']['created_by'])))
                {
                    $Model->data['Attachment']['created_by'] = $loggedUser["id"];
                }
            }
        }
        return $Model;
