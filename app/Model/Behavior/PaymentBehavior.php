...
class PaymentBehavior extends ModelBehavior
{
    ...
    private function setCreatedUpdated($Model)
    {
        ...
        if (isset($loggedUser))
        {
            if ($Model->id)
            {
                $Model->data['Payment']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Payment']['updated_by'])))
                {
                    $Model->data['Payment']['updated_by'] = $loggedUser["id"];
                }
            } else
            {
                $Model->data['Payment']['created'] = date("Y-m-d H:i:s");
                $Model->data['Payment']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Payment']['updated_by'])))
                {
                    $Model->data['Payment']['updated_by'] = $loggedUser["id"];
                }
                if (!(isset($Model->data['Payment']['created_by'])))
                {
                    $Model->data['Payment']['created_by'] = $loggedUser["id"];
                }
            }
        }
        return $Model;
