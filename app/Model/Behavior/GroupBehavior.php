...
class GroupBehavior extends ModelBehavior
{
    ...
    private function setCreatedUpdated($Model)
    {
        ...
        if (isset($loggedUser))
        {
            if ($Model->id)
            {
                $Model->data['Group']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Group']['updated_by'])))
                {
                    $Model->data['Group']['updated_by'] = $loggedUser["id"];
                }
            } else
            {
                $Model->data['Group']['created'] = date("Y-m-d H:i:s");
                $Model->data['Group']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Group']['updated_by'])))
                {
                    $Model->data['Group']['updated_by'] = $loggedUser["id"];
                }
                if (!(isset($Model->data['Group']['created_by'])))
                {
                    $Model->data['Group']['created_by'] = $loggedUser["id"];
                }
            }
        }
        return $Model;
