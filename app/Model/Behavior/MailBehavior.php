...
class MailBehavior extends ModelBehavior
{
    ...
    private function setCreatedUpdated($Model)
    {
        ...
        if (isset($loggedUser))
        {
            if ($Model->id)
            {
                $Model->data['Mail']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Mail']['updated_by'])))
                {
                    $Model->data['Mail']['updated_by'] = $loggedUser["id"];
                }
            } else
            {
                $Model->data['Mail']['created'] = date("Y-m-d H:i:s");
                $Model->data['Mail']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Mail']['updated_by'])))
                {
                    $Model->data['Mail']['updated_by'] = $loggedUser["id"];
                }
                if (!(isset($Model->data['Mail']['created_by'])))
                {
                    $Model->data['Mail']['created_by'] = $loggedUser["id"];
                }
            }
        }
        return $Model;
