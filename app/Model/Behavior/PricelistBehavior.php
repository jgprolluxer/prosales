...
class PricelistBehavior extends ModelBehavior
{
    ...
    private function setCreatedUpdated($Model)
    {
        ...
        if (isset($loggedUser))
        {
            if ($Model->id)
            {
                $Model->data['Pricelist']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Pricelist']['updated_by'])))
                {
                    $Model->data['Pricelist']['updated_by'] = $loggedUser["id"];
                }
            } else
            {
                $Model->data['Pricelist']['created'] = date("Y-m-d H:i:s");
                $Model->data['Pricelist']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Pricelist']['updated_by'])))
                {
                    $Model->data['Pricelist']['updated_by'] = $loggedUser["id"];
                }
                if (!(isset($Model->data['Pricelist']['created_by'])))
                {
                    $Model->data['Pricelist']['created_by'] = $loggedUser["id"];
                }
            }
        }
        return $Model;
