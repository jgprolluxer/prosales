...
class ActivityBehavior extends ModelBehavior
{
    ...
    private function setCreatedUpdated($Model)
    {
        ...
        if (isset($loggedUser))
        {
            if ($Model->id)
            {
                $Model->data['Activity']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Activity']['updated_by'])))
                {
                    $Model->data['Activity']['updated_by'] = $loggedUser["id"];
                }
            } else
            {
                $Model->data['Activity']['created'] = date("Y-m-d H:i:s");
                $Model->data['Activity']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Activity']['updated_by'])))
                {
                    $Model->data['Activity']['updated_by'] = $loggedUser["id"];
                }
                if (!(isset($Model->data['Activity']['created_by'])))
                {
                    $Model->data['Activity']['created_by'] = $loggedUser["id"];
                }
            }
        }
        return $Model;
