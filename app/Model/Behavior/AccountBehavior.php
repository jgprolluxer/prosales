...
class AccountBehavior extends ModelBehavior
{
    ...
    private function setCreatedUpdated($Model)
    {
        ...
        if (isset($loggedUser))
        {
            if ($Model->id)
            {
                $Model->data['Account']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Account']['updated_by'])))
                {
                    $Model->data['Account']['updated_by'] = $loggedUser["id"];
                }
            } else
            {
                $Model->data['Account']['created'] = date("Y-m-d H:i:s");
                $Model->data['Account']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Account']['updated_by'])))
                {
                    $Model->data['Account']['updated_by'] = $loggedUser["id"];
                }
                if (!(isset($Model->data['Account']['created_by'])))
                {
                    $Model->data['Account']['created_by'] = $loggedUser["id"];
                }
            }
        }
        return $Model;
