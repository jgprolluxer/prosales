...
class ReportBehavior extends ModelBehavior
{
    ...
    private function setCreatedUpdated($Model)
    {
        ...
        if (isset($loggedUser))
        {
            if ($Model->id)
            {
                $Model->data['Report']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Report']['updated_by'])))
                {
                    $Model->data['Report']['updated_by'] = $loggedUser["id"];
                }
            } else
            {
                $Model->data['Report']['created'] = date("Y-m-d H:i:s");
                $Model->data['Report']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Report']['updated_by'])))
                {
                    $Model->data['Report']['updated_by'] = $loggedUser["id"];
                }
                if (!(isset($Model->data['Report']['created_by'])))
                {
                    $Model->data['Report']['created_by'] = $loggedUser["id"];
                }
            }
        }
        return $Model;
