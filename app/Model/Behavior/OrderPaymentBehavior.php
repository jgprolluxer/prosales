...
class OrderPaymentBehavior extends ModelBehavior
{
    ...
    private function setCreatedUpdated($Model)
    {
        ...
        if (isset($loggedUser))
        {
            if ($Model->id)
            {
                $Model->data['OrderPayment']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['OrderPayment']['updated_by'])))
                {
                    $Model->data['OrderPayment']['updated_by'] = $loggedUser["id"];
                }
            } else
            {
                $Model->data['OrderPayment']['created'] = date("Y-m-d H:i:s");
                $Model->data['OrderPayment']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['OrderPayment']['updated_by'])))
                {
                    $Model->data['OrderPayment']['updated_by'] = $loggedUser["id"];
                }
                if (!(isset($Model->data['OrderPayment']['created_by'])))
                {
                    $Model->data['OrderPayment']['created_by'] = $loggedUser["id"];
                }
            }
        }
        return $Model;
