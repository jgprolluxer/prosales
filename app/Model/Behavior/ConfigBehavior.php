...
class ConfigBehavior extends ModelBehavior
{
    ...
    private function setCreatedUpdated($Model)
    {
        ...
        if (isset($loggedUser))
        {
            if ($Model->id)
            {
                $Model->data['Config']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Config']['updated_by'])))
                {
                    $Model->data['Config']['updated_by'] = $loggedUser["id"];
                }
            } else
            {
                $Model->data['Config']['created'] = date("Y-m-d H:i:s");
                $Model->data['Config']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Config']['updated_by'])))
                {
                    $Model->data['Config']['updated_by'] = $loggedUser["id"];
                }
                if (!(isset($Model->data['Config']['created_by'])))
                {
                    $Model->data['Config']['created_by'] = $loggedUser["id"];
                }
            }
        }
        return $Model;
