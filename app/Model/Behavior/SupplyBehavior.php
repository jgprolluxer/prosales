...
class SupplyBehavior extends ModelBehavior
{
    ...
    private function setCreatedUpdated($Model)
    {
        ...
        if (isset($loggedUser))
        {
            if ($Model->id)
            {
                $Model->data['Supply']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Supply']['updated_by'])))
                {
                    $Model->data['Supply']['updated_by'] = $loggedUser["id"];
                }
            } else
            {
                $Model->data['Supply']['created'] = date("Y-m-d H:i:s");
                $Model->data['Supply']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Supply']['updated_by'])))
                {
                    $Model->data['Supply']['updated_by'] = $loggedUser["id"];
                }
                if (!(isset($Model->data['Supply']['created_by'])))
                {
                    $Model->data['Supply']['created_by'] = $loggedUser["id"];
                }
            }
        }
        return $Model;
