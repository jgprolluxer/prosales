...
class ProductBehavior extends ModelBehavior
{
    ...
    private function setCreatedUpdated($Model)
    {
        ...
        if (isset($loggedUser))
        {
            if ($Model->id)
            {
                $Model->data['Product']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Product']['updated_by'])))
                {
                    $Model->data['Product']['updated_by'] = $loggedUser["id"];
                }
            } else
            {
                $Model->data['Product']['created'] = date("Y-m-d H:i:s");
                $Model->data['Product']['updated'] = date("Y-m-d H:i:s");
                if (!(isset($Model->data['Product']['updated_by'])))
                {
                    $Model->data['Product']['updated_by'] = $loggedUser["id"];
                }
                if (!(isset($Model->data['Product']['created_by'])))
                {
                    $Model->data['Product']['created_by'] = $loggedUser["id"];
                }
            }
        }
        return $Model;
