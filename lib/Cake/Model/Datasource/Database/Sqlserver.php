		...
		switch (strtolower($type)) {
			case 'select':
				...
				if (strpos($limit, 'FETCH') !== false) {
					...
					$sql = trim("SELECT {$fields} FROM {$table} {$alias} {$joins} {$conditions} {$group} {$order} {$limit}");
					$sql = $this->MySQL2MsSQL($sql);
					return $sql;
				}

				$sql = trim("SELECT {$limit} {$fields} FROM {$table} {$alias} {$joins} {$conditions} {$group} {$order}");
				$sql = $this->MySQL2MsSQL($sql);
				return $sql;
			...
			default:
			{
				//2014.09.30 SFICHERA: Added in order to properly support null values for int fields..
				if($type == "create") {
					$values = $data["values"];
					$values =  str_replace("'NULL',", "NULL,", $values);
					$values =  str_replace("NNULL,", "N'NULL',", $values);
...
/**
 * Helper function in order to convert MySQL functions to SQLServer functions...
 *
 * @param string $value SQL statement
 * @return string The modified SQL statement
 */
	private function MySQL2MsSQL($value) {
		if (strpos($value, 'IF(') !== FALSE) {
			$value = (str_replace( "IF(", "IIF(", $value ));
		}
		if (strpos($value, 'IFNULL(') !== FALSE) {
			$value = (str_replace( "IFNULL(", "ISNULL(", $value ));
		}
		if (strpos($value, '`') !== FALSE) {
			$value = (str_replace( "`", "", $value ));
		}
		if (strpos($value, "CONCAT('$',FORMAT(") !== FALSE) {
			$value = (str_replace( "FORMAT(", "CONVERT(decimal(10,2),", $value ));
		}
		if (strpos($value, "CONCAT('$',(SELECT FORMAT(") !== FALSE) {
			$value = (str_replace( "FORMAT(", "CONVERT(decimal(10,2),", $value ));
		}

		if (strpos($value, "Order.") !== FALSE) {
			$value = (str_replace( "Order.", "[Order].", $value ));
		}
		if (strpos($value, "Group.") !== FALSE) {
			$value = (str_replace( "Group.", "[Group].", $value ));
		}
		if (strpos($value, "User.") !== FALSE) {

			$value = (preg_replace('/\bUser.\b/', '[User].', $value));
		}
		if (strpos($value, "DATE_FORMAT(") !== FALSE) {
			$value = (str_replace( "DATE_FORMAT(", "", $value ));
			$value = (str_replace( ",'%Y-%m-%d %H:%i')", "", $value ));
			$value = (str_replace( ",'%Y-%m-%d')", "", $value ));
		}
		if (strpos($value, "UNIX_TIMESTAMP") !== FALSE) {
			$value = (str_replace( "UNIX_TIMESTAMP", "dbo.UNIX_TIMESTAMP", $value ));
		}
		if (strpos($value, "sysdate()") !== FALSE) {
			$value = (str_replace( "sysdate()", "GETDATE()", $value ));
		}	
		if (strpos($value, "= 'null'") !== FALSE) {
			$value = (str_replace( "= 'null'", "IS NULL", $value ));
		}		
		return($value);
	}
